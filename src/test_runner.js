...
class TestRunner {
  ...
  runTest(test, worker) {
    ...
    try {
      ...
      let seleniumPort = worker.portOffset + 1;
      // if executor has its own port rule
      if (test.executor.getSeleniumPort
        && typeof test.executor.getSeleniumPort === "function") {
        seleniumPort = test.executor.getSeleniumPort();
      }
